...
\noindent\textbf{Solution:}
\\
\noindent\textbf{核心转换思路}
将每个顶点拆分为"入顶点"和"出顶点"两个节点，通过一条容量等于原顶点容量的边连接它们，从而把顶点容量约束转化为边容量约束。

\noindent\textbf{具体转换步骤}
\begin{enumerate}
    \item \textbf{顶点拆分}：对原网络中每个顶点 $v \in V$，拆分为两个顶点 $v_{\text{in}}$（负责接收所有流入原顶点 $v$ 的流量）和 $v_{\text{out}}$（负责发送所有从原顶点 $v$ 流出的流量）。
    \item \textbf{添加顶点容量边}：在 $v_{\text{in}}$ 和 $v_{\text{out}}$ 之间添加有向边 $v_{\text{in}} \to v_{\text{out}}$，其容量等于原顶点 $v$ 的容量 $c(v)$。
    \item \textbf{重定向原有边}：将原网络中每条有向边 $u \to v$（容量为 $c(u,v)$）替换为有向边 $u_{\text{out}} \to v_{\text{in}}$，容量保持不变。
\end{enumerate}

\noindent\textbf{等价性证明}
\begin{itemize}
    \item 对于 $G'$ 中的任意合法流 $f'$，可构造 $G$ 中的合法流 $f$：令 $f(u,v) = f'(u_{\text{out}}, v_{\text{in}})$。由于 $f'(v_{\text{in}}, v_{\text{out}}) \leq c(v)$，因此通过原顶点 $v$ 的总流量也不超过 $c(v)$，满足顶点容量约束。
    \item 对于 $G$ 中的任意合法流 $f$，可构造 $G'$ 中的合法流 $f'$：令 $f'(u_{\text{out}}, v_{\text{in}}) = f(u,v)$，且 $f'(v_{\text{in}}, v_{\text{out}})$ 等于通过原顶点 $v$ 的总流量。由于 $f$ 满足顶点容量约束，因此 $f'(v_{\text{in}}, v_{\text{out}}) \leq c(v)$，满足边容量约束。
\end{itemize}
因此，$G$ 和 $G'$ 的最大流值完全相等。

\noindent\textbf{顶点和边的数量}
\begin{itemize}
    \item 顶点数量：原网络有 $|V|$ 个顶点，每个拆分为2个，故 $|V'| = 2|V|$。
    \item 边数量：原网络有 $|E|$ 条边（每条替换为1条），加上每个顶点拆分产生的1条边，故 $|E'| = |E| + |V|$。
\end{itemize}
...
这两个问题可以转换为两个新表述：
\begin{enumerate}
    \item 对于问题1，是否存在$k$条边不相交且起点终点一一对应的路径。
    \item 对于问题2，是否存在$k$条顶点不相交且边也不相交，同时起点终点一一对应的路径。
\end{enumerate}


\noindent\textbf{1. 边不相交路径算法}
\noindent\textbf{算法输入输出}
\textbf{输入}：有向图$G=(V,E)$，起点集$A=\{a_1,\dots,a_k\}$，终点集$B=\{b_1,\dots,b_k\}$

\textbf{输出}：若存在，返回$k$条边不相交且起点终点一一对应的路径；否则返回不存在

\noindent\textbf{算法步骤}
\begin{enumerate}
    \item 构造流网络$G_1=(V_1,E_1,c_1)$：
    \begin{itemize}
        \item 新增源点$s$和汇点$t$，令$V_1 = V \cup \{s, t\}$
        \item 对原图中每条边$(u,v) \in E$，在$G_1$中添加边$(u,v)$，容量$c_1(u,v)=1$
        \item 对每个起点$a_i \in A$，添加边$(s,a_i)$，容量$c_1(s,a_i)=1$
        \item 对每个终点$b_j \in B$，添加边$(b_j,t)$，容量$c_1(b_j,t)=1$
    \end{itemize}
    
    \item 使用Dinic算法计算从$s$到$t$的最大流$f$
    
    \item 若最大流的流量值$|f| \neq k$，则返回不存在满足条件的路径
    
    \item 初始化路径集合$P = \emptyset$
    
    \item 对每个起点$a_i \in A$：
    \begin{itemize}
        \item 若边$(s,a_i)$上的流量$f(s,a_i) = 1$
        \item 从$a_i$出发，沿流量为1的边进行深度优先搜索（DFS）或广度优先搜索（BFS），找到对应的终点$b_j$
        \item 将路径$a_i \to \dots \to b_j$加入集合$P$
    \end{itemize}
    
    \item 返回路径集合$P$
\end{enumerate}

\noindent\textbf{2. 顶点不相交路径算法}

\noindent\textbf{算法输入输出}

\textbf{输入}：

有向图$G=(V,E)$，起点集$A=\{a_1,\dots,a_k\}$，终点集$B=\{b_1,\dots,b_k\}$

\textbf{输出}：

若存在，返回$k$条顶点不相交且边不相交，同时起点终点一一对应的路径；否则返回不存在

\noindent\textbf{算法步骤}
\begin{enumerate}
    \item 进行顶点分裂，构造中间图$G'=(V',E')$：
    \begin{itemize}
        \item 对每个顶点$v \in V$，拆分为入顶点$v_{in}$和出顶点$v_{out}$
        \item 令$V' = \{v_{in}, v_{out} \mid v \in V\}$
        \item 对每个顶点$v \in V$，添加边$(v_{in},v_{out})$，容量$c'(v_{in},v_{out})=1$
        \item 对原图中每条边$(u,v) \in E$，修改为边$(u_{out},v_{in})$，容量$c'(u_{out},v_{in})=1$
    \end{itemize}
    
    \item 构造最终流网络$G_2=(V_2,E_2,c_2)$：
    \begin{itemize}
        \item 新增源点$s$和汇点$t$，令$V_2 = V' \cup \{s, t\}$
        \item 对每个起点$a_i \in A$，添加边$(s,a_{i,in})$，容量$c_2(s,a_{i,in})=1$
        \item 对每个终点$b_j \in B$，添加边$(b_{j,out},t)$，容量$c_2(b_{j,out},t)=1$
    \end{itemize}
    ...
    \item 对每个起点$a_i \in A$：
    \begin{itemize}
        \item 若边$(s,a_{i,in})$上的流量$f(s,a_{i,in}) = 1$
        \item 从$a_{i,in}$出发，沿流量为1的边遍历，得到路径：
        \[ s \to a_{i,in} \to a_{i,out} \to u_{in} \to u_{out} \to \dots \to b_{j,in} \to b_{j,out} \to t \]
        \item 将路径中的顶点对$(v_{in},v_{out})$合并为原顶点$v$，得到原路径$a_i \to \dots \to b_j$
        \item 将该路径加入集合$P$
    \end{itemize}
